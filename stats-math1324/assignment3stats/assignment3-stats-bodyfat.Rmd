...
```{r echo = F, message=F, warnings=0, results='hide'}
...
```{r echo=TRUE}
body %>% .[-c(1,3:5,length(.))] -> bodyCorrelation # remove sex and case number for correlation
bivariate <- as.matrix(bodyCorrelation)  %>% round(digits = 2)
# Correlation
# cor(bivariate)
# P Variable
rcorr(x = bivariate, type = "pearson")
```

```{r}

# getCorrelation <- function(.) {
#   bivariate <- bodyCorrelation %>% colnames() %>% as.matrix()
#   rcorr(bivariate, type = "pearson")
#   foo <- .[, lapply(., is.numeric) %>% unlist()] %>%
#     rcorr(foo, type = "pearson") %>% round(digits = 2)
#   foo %>% as.data.frame()
# }

# This orders by highest bodyCorrelation, we don't need this
bodyCorrelation %>% .[order(-bodyCorrelation$BFP_Brozek), 1:15]
# [1:9,1:9]
...
pairs.panels(bodyCorrelation[,c(1:15)],method = "pearson", hist.col = "#00AFBB", density = TRUE,  ellipses = TRUE, cor =T, ci=T, digits = 2,rug = F,breaks = 20, stars = T)
...
bodyCorrelation %>% getCorrelation()
...
#  bodyCorrelations[order(-bodyCorrelations$BFP_Brozek), 2:10][1:9,1:9]
# bodyCorrelations
# 
# orderedCorrelation <-
#   bodyCorrelations[,1:19] %>%
...
# 
# 
# pairs.panels(bodyCorrelations[,c(2,5:10)],method = "pearson", hist.col = "#00AFBB", density = TRUE,  ellipses = TRUE, cor =T, ci=T, digits = 2,rug = F,breaks = 20, stars = T)
# # pairs.panels(orderedCorrelation[,c(1,3:12)], method="pearson", hist.col="#00AFBB", density=T, ellipses=T, cor =T, ci=T)

```

```{r}
# bivariate<-as.matrix(select(body, ~Height)) #Create a matrix of the variables to be correlated
# 
# qqPlot(body$BFP_Brozek, dist="norm") #Check normality if required # gives a weird answer
# rcorr(bivariate, type = "pearson") #Create a matrix of the variables to be correlated
# 
