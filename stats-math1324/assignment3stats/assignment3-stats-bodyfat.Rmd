...
bivariate <- bodyCorrelation %>% as.matrix() # TODO: possibly refactor this
unorderedBivariateRcorr <- bivariate %>% rcorr(type = "pearson") # Unordered

# Order our factors by Pearson correlation with with BFP_Brozek
bodyCorrelationFactorOrder <- unorderedBivariateRcorr$r %>% as.data.frame() %>% .[1] %>% order(decreasing = T)
bodyCorrelation <- bodyCorrelation[bodyCorrelationFactorOrder] # TODO: refactor this if I don't know bodyCorrelation
bodyCorrelationOrderedByRcorr <- bodyCorrelation %>% as.matrix() %>% rcorr(type = "pearson") # Ordered R Correlation

# Find the single best predictor of body fat percentage (Brozek method) using the body circumference data.
#bodyCorrelationOrderedByRcorr$r %>% colnames() # Here they are by Pearson correlation in order
bodyCorrelationOrderedByRcorr
bodyCorrelationOrderedByRcorr$r %>% corrplot(method = "circle", type = "upper", tl.col = "darkblue", tl.srt = 45,  p.mat=bodyCorrelationOrderedByRcorr$p, sig.level = 1, insig = "blank")
