...
body <- read_excel("data/Body (2) Assignment 3 data.xlsx", sheet = "Body (2) Assignment 3 data")
body <- mutate(body, Sex=factor(Sex, labels=c("female", "male"))) %>% as.data.frame()
body %>% head()
...
### 1. Test whether the mean body fat percentage for males and females are the same.
...
### null hypothesis
TODO:
Females have a higher % body fat
...
#### 2. Estimate the 99% confidence interval for the mean body fat percentage in the population.

```{r}
body$BFP_Brozek %>% mean()

binom.approx(44, 109, conf.level = 0.99)

binom.approx()

confint(t.test(~ BFP_Brozek, data = male))
confint(t.test(~ BFP_Brozek, data = female))
...
#### 3. Researchers believe that average body fat percentage is less than 12.5.
