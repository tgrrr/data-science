...
library(magrittr)
library(TSA)
working_directory <- "~/code/data-science/cheatsheet/source/includes/time-series/raw-data"
...
```{r}
# label the linear trend model as model1
...
#### Use least squares to fit a linear time trend to this time series. Interpret the regression output. 

```{r}
t = time(wages)
t2 = t ^ 2
model2 = lm(wages ~ t + t2) # label the quadratic trend model as model2
summary(model2)
...
#### Save the standardised residuals from the fit for further analysis.
