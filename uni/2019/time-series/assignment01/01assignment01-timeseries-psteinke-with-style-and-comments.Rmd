...
We found that ... ~old_todo~~old_later~
...
```
- This plot shows a negative decreasing simple moving average in thickness of ozone layer over 90 years. 
...
doTimeSeriesPlot(
  ...
  title = 'Standardised Residuals of Linear Trends model with Simple moving average',
  lines=NULL)
```
...
Q-Q plot shows significant skew indicating possible non-normality or bimodal data


```{r}
shapiro.test(res.linearTrendModel)
```
...
Visual inspection shows a closer fit to the data
...
qqline(
  ...
  lwd = 1)
```
Q-Q plot has some skew towards the end. However, it is less skewed than the linear model

```{r}
acf(res.model.ozone.qa)
```
Shows 3 clear and 5 total outliers outside of the 0.2
...
**Task:** Find the best fitting trend model to this dataset
...
The data shows an increased rate of decay over time. This does not appear to be a stationary series, with seasonal pattern and moving average. It would be ideal to find a better fit is warranted.
