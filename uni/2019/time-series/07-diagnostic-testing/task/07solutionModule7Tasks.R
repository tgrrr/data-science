...
# 1. The dataset given in “unemployment.Preview the documentcsv” file contains yearly averages of monthly total number of unemployed people (in thousands) in Australia between 1978 and 2016. This dataset is available through http://www.abs.gov.au/AUSSTATS/abs@.nsf/DetailsPage/6202.0Feb%202017?OpenDocument#Time (Links to an external site.)Links to an external site. webpage. Load the dataset, convert it into a ts object and do the following tasks:
# 
...
unemployment <- read.table("~/code/data-science/uni/time-series/raw-data/unemployment.csv", quote="\"", comment.char="")
...
# {ARIMA(2,2,1)}
# getting 2 lags from acf, 1 lag from pacf, and it's diff 2 
# => (arima(p,d,q) == (2,2,1) )
...
# 2. Load the simulated dataset in “data.simPreview the document.Preview the documentcsv” file convert it into a ts object and do the following tasks:
# Specify a set of tentative ARIMA models including at least three models by using
# ACF and PACF plots,
# EACF plot, and
# BIC table
