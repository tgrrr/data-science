# see the original for questions
...
data <- read.table(
  "~/code/data-science/uni/time-series/raw-data/unemployment.csv", 
  quote="\"", comment.char="")

data = ts(data,start = 1978)
class(data)
data.raw = data


plot(data,type='o',ylab='Time series plot of data series')

acf(data)
pacf(data)

data.transform = BoxCox.ar(data)
data.transform = BoxCox.ar(data, method = "yule-walker")
data.transform$ci

data = log(data) # Since 0 is in the interval. Alternatively you can go for the mid point as well.
plot(data,type='o',ylab='Time series plot of log of data series')


diff.data = diff(data, differences = 1) # Be careful I'm using log-transformed series
order = ar(diff(diff.data))$order
adfTest(diff.data, lags = order,  title = NULL,description = NULL)
plot(diff.data,type='o',ylab='QThe first difference of data series')

diff.data = diff(data, differences = 2) # Be careful I'm using log-transformed series
order = ar(diff(diff.data))$order
adfTest(diff.data, lags = order,  title = NULL,description = NULL)
plot(diff.data,type='o',ylab='The second difference of data series')

acf(diff.data)
pacf(diff.data)
# {ARIMA(2,2,1)}

eacf(diff.data,ar.max = 7, ma.max = 7)
# {ARIMA(2,2,1), ARIMA(0,2,1), ARIMA(1,2,1)}

res = armasubsets(y=diff.data,nar=6,nma=6,y.name='test',ar.method='ols')
plot(res)

#The final set of possible models is {ARIMA(2,2,1), ARIMA(0,2,1), ARIMA(1,2,1)}

# ARIMA(2,2,1)
model_221_css = arima(data,order=c(2,2,1),method='CSS')
coeftest(model_221_css)

model_221_ml = arima(data,order=c(2,2,1),method='ML')
...
log.data = log(data.raw)
log.data.diff2.back = diffinv(diff.data, differences = 2, xi = data.matrix(log.data[1:2]))
...
fit = Arima(data.raw,c(1,2,1), lambda = 0) 
