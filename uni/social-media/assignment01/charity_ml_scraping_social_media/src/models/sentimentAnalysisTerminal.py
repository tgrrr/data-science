...
class Constants:
    path = "/Users/phil/code/data-science-next/uni/social-media/assignment_1/charity_ml_scraping_social_media/"
...
def doSentimentAnalysis(
                    ...
                    if bPrint:
                        ...
                        for token in lTokens:
                            if token in setPosWords:
                                _pos = ['\+', token ]
                                word_cloud_out.append(_pos)
                                print(Fore.RED + separator.join(_pos))
                            elif token in setNegWords:
                                _neg = ['\-', token ]
                                word_cloud_out.append(neg)
                                print(Fore.BLUE +separator.join(_neg))
                            else:
                                _noSentiment = token
                                word_cloud_out.append(_noSentiment)
                                print(Style.RESET_ALL + _noSentiment)

                        # write wordcloud to text file - Phil
                        f = open("data/sentiment_wordcloud_out.txt")
                        f.write(word_cloud_out)
    ...
    # approach = 'count' MOVED TO FUNCTION
    ...
    # FIXME: update 
    plt.savefig('reports/figures/charity_sentiment_analysis_timeseries_' + approach + '.png')
